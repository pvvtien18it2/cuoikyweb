...
<html lang="en">
    ...
    <body>
        ...
        <div class="container">
            <div class="row">
                <h2 style="text-align: center ; font-size: 70px ; padding: 20px; margin: auto">Danh sách đặt phòng trước
                </h2>
                <table class="table table-bordered table-hover">
                    ...
                    $times = $timeOld->diffInHours($now);
                    @endphp
                    @if ($days < 0) @if($now->isToday($timeOld) && $timeOld->isPast())
                        @php
                        ...
                        @endphp
                        {{-- @if( $offset == $times) --}}
                        @if(in_array($c->phong_id , $arr) && in_array($times , $offsets) && $days < 1 ) @php
